...
public class Menu extends Model<Menu> {
	...
	public List<Menu> findAll() {
		return find("select * from sys_menu order by parent_id, order_num asc");
	}
	...
	public List<Menu> findByUser(SysUser user) {
		...
		if(StringUtil.isNotEmpty(roles)) {
			result = find("select distinct b.* from sys_role_menu a join sys_menu b on a.menu_id=b.id where a.role_id in(" + roles + ") order by b.order_num asc");
		}
		return result;
	}
